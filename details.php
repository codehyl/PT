...
elseif ($row['banned'] == 'yes' && get_user_class() < $seebanned_class)
	...
	if (!isset($_GET["cmtpage"])) {
        ...
        tr('下载链接','<a href="'.get_protocol_prefix().$BASEURL.'/download.php?id='.$id.'&passkey='.$CURUSER['passkey'].'">'.get_protocol_prefix().$BASEURL.'/download.php?id='.$id.' [右键复制，已隐藏passkey]</a>',1);
